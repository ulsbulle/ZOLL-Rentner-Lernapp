...
async function loadDownloadFiles() {
    ...
    try {
        ...
        templateList.innerHTML = csvFiles.map(file => `
            <button onclick="loadTemplate('public/templates/${file}')" class="w-full p-4 border-2 rounded-xl bg-white hover:border-blue-500 hover:bg-blue-50 text-left font-bold transition-all flex justify-between items-center group">
