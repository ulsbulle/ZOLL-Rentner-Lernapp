...
export class BubbleGame extends MiniGame {
	...
	blow() {
		...
		if (distance < this.target.r + 10 * S) {
			...
			if (this.target.r > PHYSICS.bubble.burstSize) {
				...
				this.engine.audio.playSound(SOUNDS.bubble.bubbleBurst);
	...
	update(deltaTime) {
		...
		// Bewegung & Bewegungsumkehr
		this.target.x += this.target.speedX * deltaTime;
		this.target.y += this.target.speedY * deltaTime;
		if (this.target.x < this.target.r || this.target.x > W - this.target.r) this.target.speedX *= -1;
		if (this.target.y < this.target.r || this.target.y > H - this.target.r) this.target.speedY *= -1;

		// Platzen
		if (this.target.r < 5 * S) {
			this.engine.applyDamage(POINTS.bubble.bubbleImplosion);
			this.engine.audio.playSound(SOUNDS.bubble.bubbleImplosion);
			this.createBubble();
		}
	}
