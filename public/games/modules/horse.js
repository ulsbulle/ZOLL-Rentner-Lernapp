...
export class HorseGame extends MiniGame {
	...
	init() {
		this.player = { x: W / 2, y: PHYSICS.horse.groundY, speedY: 0, isJumping: false, jumpHoldFrames: 0 };
		this.clouds = [];
		this.obstacles = [];
	...
	jump(jumpPower = undefined) {
		if (this.player.y >= PHYSICS.horse.groundY) {
			...
			this.engine.audio.playSound(SOUNDS.horse.jump);
		}
	}
	...
	update(deltaTime) {
		...
		for (let obstacleIndex = this.obstacles.length - 1; obstacleIndex >= 0; obstacleIndex--) {
			...
			if (
				...
				this.engine.audio.playSound(SOUNDS.horse.obstacleHit);
			}

			// Hindernis übersprungen
			if (obstacle.x < 30 * S && !obstacle.passed && !obstacle.hit) {
				obstacle.passed = true;
				this.engine.addScore(POINTS.horse.obstacleCleared);
				this.engine.audio.playSound(SOUNDS.horse.obstacleCleared);
			}
